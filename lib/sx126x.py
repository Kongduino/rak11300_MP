...
class SX126X:
    def __init__(self, clkPin, mosiPin, misoPin, csPin, irqPin, rstPin, gpioPin):
        self._irq = irqPin
        self.spi = SoftSPI(baudrate=2000000, sck=Pin(clkPin), mosi=Pin(mosiPin), miso=Pin(misoPin))
        self.cs = Pin(csPin, mode=Pin.OUT)
        self.irq = Pin(irqPin, mode=Pin.IN)
        self.rst = Pin(rstPin, mode=Pin.OUT)
        self.gpio = Pin(gpioPin, mode=Pin.IN)
        self._bwKhz = 0
        self._sf = 0
        self._bw = 0
        self._cr = 0
    ...
    def SPItransfer(self, cmd, cmdLen, write, dataOut, dataIn, numBytes, waitForBusy, timeout=5000):
        ...
        if write:
            for i in range(numBytes):
                ...
                if (in_[0] & 0b00001110) == SX126X_STATUS_CMD_TIMEOUT or (in_[0] & 0b00001110) == SX126X_STATUS_CMD_INVALID or (in_[0] & 0b00001110) == SX126X_STATUS_CMD_FAILED:
